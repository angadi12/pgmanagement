...
import { fetchFloorsByBranch, fetchSingleTenant ,fetchTenantsByBranch} from "../../lib/TennatSlice";
...
import { Changeroomapi } from "@/lib/API/Tennat";
import {GetRoomsbyroomid} from "../../lib/API/Room"
import toast, { Toaster } from "react-hot-toast";

const isValidObjectId = (id) => /^[0-9a-fA-F]{24}$/.test(id);

const Changeroom = ({Setchangeroom}) => {
  ...
  const disabledRoomKeys = availableRooms
    ...
    const handlechangeroom = async () => {
        if (!selectedRoomId || !selectedTenantId) {
          return toast.error("Please select both a tenant and a room.");
        }
    
        setLoadingChangeRoom(true);
        try {
          const response = await Changeroomapi(selectedTenantId, selectedRoomId);
          if (response.status) {
            dispatch(fetchTenantsByBranch(selectedBranchId))
            Setchangeroom(false)
          } else {
            toast.error(response.message || "Failed to change room.");
          }
        } catch (error) {
          toast.error("An error occurred while changing the room.");
        } finally {
  ...
  return (
   ...
   {loadingRoomDetails ?<div className="flex justify-center items-center w-full h-80"><span className="loader3"></span></div>: <div className="flex flex-col justify-center items-center gap-4">
      ...
      <div className="flex justify-center items-center w-full">
        ...
        >
         { loadingChangeRoom?<span className="loader2"></span>:"Change Room"}
        </Button>
      </div>
    </div>}
