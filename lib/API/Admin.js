...
export const GetAdminbybranch = async (branchid) => {
  ...
  try {
    ...
    throw new Error(error.message || " Error while frtching Admin ");
  }
};
...
export const Updateadminstatusapi = async (selectedAdminid) => {
    const token = Cookies.get("token");
    try {
      let result = await fetch(`${BaseUrl}/admin/update/active/admin/${selectedAdminid}`, {
        method: "PUT",
        headers: {
          "Content-type": "application/json",
          token: token,
